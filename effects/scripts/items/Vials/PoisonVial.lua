...
return item.init{
    desc  = function ()
        return {
            ...
            name      = RPD.StringsManager:maybeId("PoisonVial_Name"),
            info      = RPD.StringsManager:maybeId("PoisonVial_Desc"),
            stackable = true,
            price     = 20
        }
    end,
           ...
           for i = x - 1, x + 1 do
             for j = y - 1, y + 1 do
               ...
               local soul =  RPD.Actor:findChar(pos)
                if soul then
                		 RPD.zapEffect(cell,pos,"Dart")
                    RPD.affectBuff(soul, RPD.Buffs.Poison , 30)
                    return
                end
             end
           end
        end
        RPD.item("Dart"):dropTo(cell)
    end
}
