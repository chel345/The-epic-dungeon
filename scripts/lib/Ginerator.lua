        ...
        if not storage.get("key") then
            ...
            if gin.kind == "Caves" or gin.kind == "Womb" then
                ...
                local function genCavesOrWomb()
                    ...
                    RPD.debug("gen l exit spawn %d %d", gin.ExitWidth, gin.ExitHeigth)
                    while (not room.canSpawnAt(r, gin.ExitWidth, gin.ExitHeigth)) do
                        r = math.random(1, l - 2)
                        genSteps = genSteps + 1
                        if genSteps > genStepsMax then
                            return false
                        end
                    end
                    ...
                    if gin.Shop and gin.LevelShop == RPD.Dungeon.depth then
                        r = math.random(1, l - 1)
                        RPD.debug("gen l shop spawn %d %d", gin.NPCRoomWidth, gin.NPCRoomHeigth)
                    ...
                    if gin.MiniBoss and gin.LevelMiniBoss == RPD.Dungeon.depth then
                        r = math.random(1, l - 2)
                        RPD.debug("gen l boss spawn %d %d", gin.NPCRoomWidth, gin.NPCRoomHeigth)
                        while (not room.canSpawnAt(r, gin.NPCRoomWidth, gin.NPCRoomHeigth)) do
                            r = math.random(1, l - 2)
                            genSteps = genSteps + 1
                            if genSteps > genStepsMax then
                    ...
                    if gin.NPCRoom and gin.levelNPC == RPD.Dungeon.depth then
                        r = math.random(1, l - 2)
                        RPD.debug("gen l npc spawn %d %d", gin.NPCRoomWidth, gin.NPCRoomHeigth)
                        while (not room.canSpawnAt(r, gin.NPCRoomWidth, gin.NPCRoomHeigth)) do
                            r = math.random(1, l - 2)
                            genSteps = genSteps + 1
                            if genSteps > genStepsMax then
                ...
                while not genCavesOrWomb() do
                    RPD.debug("genCavesOrWomb failed for %s", file)
                end
            ...
            if gin.Items then
                for i = 1, #gin.Items do
                    local r = RPD.getCell()
                    if r > 0 then
                        local item = RPD.item(gin.Items[i])
                        RPD.debug("item: %s desc: %s", tostring(item), tostring(gin.Items[i]))
                        level:drop(item, r)
                    end
                end
            end
            ...
            level:cleanWalls()
            if spawner then
                level:addScriptedActor(RPD.new(RPD.Objects.Actors.ScriptedActor, "scripts/actors/Spawner"))
            end
